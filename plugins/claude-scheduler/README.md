...
That's it. One command. Claude runs autonomously at the scheduled time, completes the task, and sends you a clickable notification to continue the conversation.
...
```bash
brew install terminal-notifier  # For clickable notifications (macOS)
```
...
2. **Wait** → System scheduler handles timing
...
claude --plugin-dir ./claude-scheduler/plugins/claude-scheduler
```

### Dependencies

**Required:** `jq` (usually pre-installed on macOS)

**Recommended:**
```bash
brew install terminal-notifier  # Clickable notifications on macOS
```
